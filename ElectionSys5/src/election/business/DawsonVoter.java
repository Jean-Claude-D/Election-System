...
public class DawsonVoter implements Voter {
	 ...
	 * 
	 * The LocalDate date at which a DawsonVoter calls the method
	 * must be inclusively within the date range of an Election.
	 * AND
	 * if an Election is limited to a range of PostalCode postal codes,
	 * then a DawsonVoter's PostalCode postal code must be within the
	 * postal code range of an Election.
	 * 
	 * @return true if this DawsonVoter is eligible to voting in the Election election
	 * 	       false otherwise
	 * 
	 * @since 0.0.1
	 */
	...
	public boolean isEligible(Election election) {
		...
		if(!(now.isBefore(election.getStartDate())) && !(now.isAfter(election.getEndDate()))) {
			if(election.isLimitedToPostalRange()) {
				return this.postalCode.inRange(election.getPostalRangeStart(), election.getPostalRangeEnd());
			}
	 ...
	 * AND
	 * if x.compareTo(y) > 0 && y.compareTo(z), then x.compareTo(z) > 0
	 * AND
	 * if x.compareTo(y) == 0, then x.compareTo(z) == y.compareTo(z)
	 * AND
	 * if x.compareTo(y) == 0, then x.equals(y) returns true
	 * 
	 * @return a positive integer if this DawsonVoter is greater than the Voter voter
	 *         zero			      if this DawsonVoter is equal to the Voter voter
	 ...
	 * 
	 * They are of instance DawsonVoter
	 * AND
	 * Their Email email addresses are equal,
	 * according to Email's definition of equality.
	 * 
	 ...
	 * 
	 * but, if x.equals(y) == false, then x.hashCode() != y.hashCode()
