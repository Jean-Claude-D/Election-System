...
public class VoterListDB implements VoterDAO {
   ...
   * @throws DuplicateVoterException if the Voter is already in the database
   * @author Cao Hoang
   */
  ...
  /**
   * Takes an email string and looks for it in the voter database. If a match is found it returns
   * the voter who's email it belongs to. If no match found, it throws an InexistentVoterException.
   * 
  ...
  public Voter getVoter(String email) throws InexistentVoterException {
    ...
    if (ListUtilities.binarySearch(this.database, dummy) < 0) {
      throw new InexistentVoterException(
          "Sorry. There are no corresponding voters to the email you have entered.");
    }
  ...
  /**
   * Checks if the voter exists using getVoter and if it does, changes the existing postal code with
   * the one specified in the arguments of the voter that getVoter returns. If the voter does not
   * exist, getVoter throws InexistentVoterException
   * 
   * @author Felicia Gorgatchov
   * @param email : email object used to find a voter in the database
   * @param postalCode : postalCode object used to replace the voter's existing postal code with a
   *        new one
