...
public class DawsonVoterTest {
	...
	private final String PASS = "==================================================================================TEST PASSED\n";
	private final String FAIL = "-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_TEST FAILED\n";
	...
	public DawsonVoterTest(String[] validFName, String[] validLName, String[] validEmail, String[] validPCode,
			               String[] inValidFName, String[] inValidLName, String[] inValidEmail, String[] inValidPCode) {
		//Verification
		
		if(validFName == null || validLName == null || validEmail == null || validPCode == null ||
		   inValidFName == null || inValidLName == null || inValidEmail == null || inValidPCode == null) {
			throw new IllegalArgumentException("All arguments must be non-null");
		}
		
		int length = validFName.length;
		
		if(inValidLName.length != length || inValidEmail.length != length || inValidPCode.length != length ||
		   validLName.length != length || validEmail.length != length || validPCode.length != length) {
		...
		String[][] inValidFNameOnly = {this.inValidFName, this.validLName, this.validEmail, this.validPCode};
		String[][] inValidLNameOnly = {this.validFName, this.inValidLName, this.validEmail, this.validPCode};
		String[][] inValidEmailOnly = {this.validFName, this.validLName, this.inValidEmail, this.validPCode};
		String[][] inValidPCodeOnly = {this.validFName, this.validLName, this.validEmail, this.inValidPCode};
		
		this.codeBreaker = new String[][][] {inValidFNameOnly, inValidLNameOnly, inValidEmailOnly, inValidPCodeOnly};
	...
	public static void main(String[] args) {
		...
		String[] validLName = {"Desrosiers", "Gorgacheva", "Hossein", "Nguyen", "Smith", "Allen", "Hood"};
		String[] validEmail = {"jcdMyEmail@domain.name", "Fel@gmail.qc.com", "Girl@Home.Sweet.Home", "Boy@Sweet.Home.Sweet", "regularGuy123@normal.domain.name.biz", "tiny@email", "right@middle.target"};
		String[] validPCode = {"G7B 1P9", "H1W 0J8", "L4K 2Y6", "P2N 0E3", "H7G 1T3", "G3Z 3T2", "J0L 2J9"};
		
		String[] inValidFName = {"Jean---Claude", "Fe\\%l", "Maria$..", "H0ang", "R*!bert", "J??oe", "Ro#bin"};
		String[] inValidLName = {"Des++rosiers", "     ", "H0ssein", "Ngu  yen", "Sm111th", "Al1len", "H8d"};
		String[] inValidEmail = {"jcdMyEmaildomain.name", "Fel@gmail........qc.com", "", "     ", ".regularGuy123@normal.domain.name.biz", "t..iny@email", "right.middle.target"};
		String[] inValidPCode = {"G7B   1P9", "H1W 0J", "4L2 K46Y", "ZP2N 0E3", "H7G_1T3", "G3Z.3T2", "J000L 2J9"};
		
		DawsonVoterTest test = new DawsonVoterTest(validFName, validLName, validEmail, validPCode, inValidFName, inValidLName, inValidEmail, inValidPCode);
		...
		test.equalsHash();
		test.tostring();
		test.eligible(new DawsonElection("test1", "SINGLE", now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 2018, 12, 23, "", "", new StubTally(), "hello", "hi"), true);
		test.eligible(new DawsonElection("test1", "SINGLE", now.getYear(), now.getMonthValue(), now.getDayOfMonth(), 2018, 12, 23, "A", "A", new StubTally(), "hello", "hi"), false);
		test.compareto();
	...
	private void Ctor() {
		...
		try {
			...
			for(int i = 0; i < this.validVoter.length; i++) {
				this.validVoter[i] = new DawsonVoter(this.validFName[i], this.validLName[i], this.validEmail[i], this.validPCode[i]);
		...
		for(int i = 0; i < this.codeBreaker.length; i ++) {
			for(int j = 0; j < this.codeBreaker[i][0].length; j++) {
				try {
					DawsonVoter inValidVoter = new DawsonVoter(this.codeBreaker[i][0][j], this.codeBreaker[i][1][j], this.codeBreaker[i][2][j], this.codeBreaker[i][3][j]);
					System.out.println(this.codeBreaker[i][0][j] + "*" + this.codeBreaker[i][1][j] + "*" + this.codeBreaker[i][2][j] + "*" + this.codeBreaker[i][3][j]);
					System.out.println(FAIL);
				}
				catch(IllegalArgumentException e) {
					System.out.println(this.codeBreaker[i][0][j] + "*" + this.codeBreaker[i][1][j] + "*" + this.codeBreaker[i][2][j] + "*" + this.codeBreaker[i][3][j]);
	...
	private void equalsHash() {
		...
		for(int i = 0; i < this.validVoter.length; i++) {
			sameVoter = new DawsonVoter(this.validVoter[i].getName().getFirstName(), this.validVoter[i].getName().getLastName(),
					                    this.validVoter[i].getEmail().toString(), this.validVoter[i].getPostalCode().toString());
			if(sameVoter.equals(this.validVoter[i]) && sameVoter.hashCode() == this.validVoter[i].hashCode() && this.validVoter[i].equals(sameVoter) && !this.validVoter[i].equals(nullObj)) {
	...
	private void tostring() {
		...
		System.out.println();
		for(int i = 0; i < this.validVoter.length; i++) {
			if(this.validVoter[i].toString().equals(this.validVoter[i].getEmail().toString() + '*' + this.validVoter[i].getName().getFirstName() + '*'
					   + this.validVoter[i].getName().getLastName() + '*' + this.validVoter[i].getPostalCode().toString())) {
				System.out.println(this.validVoter[i]);
				System.out.println(PASS);
			}
			else {
