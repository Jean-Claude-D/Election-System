...
public class PostalCode implements Serializable, Comparable<PostalCode> {
  ...
  /**
   * constructor takes a string and runs it through a validator method. if the postal code string is
   * conform it is returned and stored in the code private final field
   * 
   * @param codeParam
   */
  ...
  /**
   * is run on a object and takes an object. compares the strings to determine if they are equal or
   * which is greater if this object is greater, return 1. if the comparison object is greater,
   * return -1. if one is not greater than the other, they must be equal, return 0.
   * 
   * @param postalCode
   */
  @Override
  ...
  /**
   * verifies that the argument object is not null. verifies if the objects are the same or if the
   * argument is a child of this class
   * 
   * @param object
   * @return boolean
   */
  ...
  public boolean equals(Object object) {
    ...
    // comparing the 2 object to see if they have the same address, in which case they would be the
    // same object referenced differently
  ...
  /**
   * follows the equals(); it returns a boolean so the calculation of the hashcode by adding
   * code.hashCode() to 1 * any prime number.
   * 
  ...
  /**
   * Returns true if this PostalCode comes after or at start and comes before or at end.
   * compareTo(PostalCode)'s ordering is used to order PostalCode from smallest to largest.
   *
   * @param start the lower boundary
   * @param end the upper boundary
   *
   * @return true if this PostalCode's code field is greater than start and smaller than end,
   *         according to lib.Name#compareTo(PostalCode)'s definition of ordering PostalCode. false
   *         otherwise.
   *
  ...
  public boolean inRange(String start, String end) {
    ...
    /*
     * Make start the smallest postal code string possible by filling up until the length of a
     * postal code e.g. if start's value is "R3", then it would fill it like so : "R3A0A0"
     */
    ...
    /*
     * Make end the largest postal code string possible by filling up until the length of a postal
     * code e.g. if end's value is "", then it would fill it like so : "Y9Z9Z9"
     */
    ...
    /*
     * Wrapping start and end in PostalCode objects allows their validation and the use of
    ...
    return this.compareTo(startPostalCode) >= 0 && this.compareTo(endPostalCode) <= 0;
   ...
   * 
   * @param code String
   * @return once the string passed all the validations and is in the correct format, the entire
   *         string is made in uppercase and if there is a space, it is removed.
  ...
  public static String validate(String code) throws IllegalArgumentException {
    ...
    // check to see if code is the right length; 6 is without a space, 7 is with a space. Less than
    // 6 and more than 7 is invalid.
    if (code.length() < 6) {
    ...
    for (int i = 0; i < code.length(); i += 2) {
      if (Character.isDigit(code.charAt(i))) {
        throw new IllegalArgumentException(
            "Invalid postal code format. Correct format: A#A #A#, where A is any letter and # is any number.");
      }
    }
    ...
    if (code.charAt(0) == 'W' || code.charAt(0) == 'Z') {
      throw new IllegalArgumentException(
          "Cannot begin by W or Z. Invalid character: " + code.charAt(0));
    }
    // Code must not contain any of these characters ('D','F','I','O','Q','U') anywhere in the
    // string. If it does, throw an exception.
